...
object3D_t makeObjTorus() {
	int n = 24; float r = 0.3, R = 0.6;
	int numofVertices = n * n; //int numofFaces = numofVertices;
	int numofFaces = n * n; float roundDigit = 1000000;
	object3D_t obj;

	//make points
	for (int i = 0; i < n; i++)
	{
		float phi = i * 2 * M_PI / n;
		for (int j = 0; j < n; j++)
		{
			float theta = j * 2 * M_PI / n;
			point[(n*i) + j].x = (R + r * cos(theta)) * cos(phi);
			point[(n*i) + j].y = (R + r * cos(theta)) * sin(phi);
			point[(n*i) + j].z = r * sin(theta);
			/*cout << roundf(point[j].x * roundDigit) / roundDigit << " "
				<< roundf(point[j].y * roundDigit) / roundDigit << " "
				<< roundf(point[j].z * roundDigit) / roundDigit << endl;*/
		}
	}	

	//make faces
	for (int i = 0, j; i < numofFaces; i++)
	{
		if (i < (n * (n-1)))
		{
			j = (i + 1) % n;
			if (j != 0)
			{
				face[i] = { 4,{i,i + 1,i + 1 + n,i + n} };
			}
			else
			{
				face[i] = { 4,{i,i - (n - 1),i + 1,i + n} };
			}
		}
		...
		else
		{
			...
			if (j < n-1)
			{
				face[i] = { 4,{i,i + 1,j + 1,j} };
			}
			else
			{
				face[i] = { 4,{i,i - (n - 1),j - (n - 1),j} };
			}
		}
	}
...
void display() {
	...
	DrawObject("D:\\Nama\\Foldernya\\Diganti\\Ya\\Gan\\torus.off");
	glutSwapBuffers();
}
